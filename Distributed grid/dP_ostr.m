...
Unom = 10; % номинальное напряжение сети [кВ]
  ...
  %v = [0.1621; 0.1256; 0.0664; 0.0850; 0.1127; 0.0236; 0.0945; 0.0732; 0.8724; 0.4063; 0.2210; 0.4142];
  %v = zeros(1, 32);
  %v = [0.2802; 0.1104; 0.1129; 0.0752; 0.0750; 0.3459];
  %v = [0.4154; 0.1894; 0.3461];
   ...
   P(2) = 0.1 - v(2);  % активная мощность нагрузки узла [МВт]
   P(3) = 0.09 - v(3);
   P(4) = 0.12 - v(4);
   P(5) = 0.06 - v(5);
   P(6) = 0.06 - v(6);
 ...
 for (k = 1:32) % вычисление активных, реактивных и полных сопротивлений линий
  ...
  for (m = 1:11) % вычисление потоков и потерь части сети от 6-го до 18-го узла
   ...
   for (m = 1:7) % вычисление потоков и потерь части сети от 6-го до 33-го узла
     ...
     Sl(2) = S(2) + Sl(18) + dS(18); % полный поток по 2-й линии [МВА]
   ...
   for (m = 1:2) % вычисление потоков и потерь части сети от 3-го до 6-го узла
     Sl(3) = S(3) + Sl(2) + dS(2) + Sl(22) + dS(22); % полный поток по 3-й линии [МВА]
     Pl(3) = real(Sl(3)); % активный поток по 3-й линии [МВт]
     Ql(3) = imag(Sl(3)); % реактивный поток по 3-й линии [МВАр]
     dS(3) = (Pl(3)^2 + Ql(3)^2)*Z(3)/U(3)^2; % полные потери мощности в 3-й линии [МВА]
       k = 3 + m; % номер линии для которой выполняется расчёт на данной итерации
       Sl(k) = S(k) + Sl(k-1) + dS(k-1); % полный поток по k-й линии [МВА]
       Pl(k) = real(Sl(k)); % активный поток по k-й линии [МВт]
       Ql(k) = imag(Sl(k)); % реактивный поток по k-й линии [МВАр]
       dS(k) = (Pl(k)^2 + Ql(k)^2)*Z(k)/U(k)^2; % полные потери мощности в k-й линии [МВА]
   end
        
  dU = zeros(1, 32); % создание нулевой матрицы потерь напряжения в линиях
  U(6) = 1.05*Unom; % напряжения базового узла [кВ]

  for (m = 1:11) % вычисление потерь напряжения в части сети от 6-го до 18-го узла [кВ]  
  dU(6) = conj(Sl(6) + dS(6))*Z(6)/conj(U(6)); % вычисление потерь напряжения в 6-й линии [кВ]  
  U(7) = U(6) - dU(6); % напряжение в узле 7 [кВ]   
  k = 6 + m; % номер линии для которой выполняется расчёт на данной итерации 
  ...
  for (m = 1:2) % вычисление потерь напряжения в части сети от 6-го до 3-го узла [кВ]  
  dU(5) = conj(Sl(5) + dS(5))*Z(5)/conj(U(6)); % вычисление потерь напряжения в 5-й линии [кВ]  
  U(5) = U(6) - dU(5); % напряжение в узле 5 [кВ]   
  k = 5 - m; % номер линии для которой выполняется расчёт на данной итерации 
  dU(k) = conj(Sl(k) + dS(k))*Z(k)/conj(U(k+1)); % вычисление потерь напряжения в k-й линии [кВ]  
  U(k) = U(k+1) - dU(k); % напряжение в узле k+1 [кВ]      
  end
  ...
  U(23) = U(3) - dU(22); % напряжение в узле 23 [кВ]   
  k = 22 + m; % номер линии для которой выполняется расчёт на данной итерации 
  dU(k) = conj(Sl(k) + dS(k))*Z(k)/conj(U(k)); % вычисление потерь напряжения в k-й линии [кВ]  
  U(k+1) = U(k) - dU(k); % напряжение в узле k+1 [кВ]      
  end
     
  dU(2) = conj(Sl(2) + dS(2))*Z(2)/conj(U(3)); % вычисление потерь напряжения во 2-й линии [кВ]  
  U(2) = U(3) - dU(2); % напряжение в узле 2 [кВ]
  
  for (m = 1:3) % вычисление потерь напряжения в части сети от 2-го до 22-го узла [кВ]  
  dU(18) = conj(Sl(18) + dS(18))*Z(18)/conj(U(2)); % вычисление потерь напряжения в 18-й линии [кВ]  
  U(19) = U(2) - dU(18); % напряжение в узле 19 [кВ]   
