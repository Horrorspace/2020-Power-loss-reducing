function Population = gacreationnonlinearfeasible(GenomeLength,~,options,ConstrFcn,varargin)
%GACREATIONNONLINEARFEASIBLE Creates the initial population for GA.
...
thisFcn = 'GACREATIONNONLINEARFEASIBLE';
parser = inputParser();
...
parser.addParameter('SolverOpts',optimoptions('fmincon','Algorithm','sqp','TolCon',options.TolCon), ...
    @(x)validateattributes(x,{'optim.options.Fmincon'}, ...
...
initPts = gacreationuniform(GenomeLength,[],options);
...
isFeas = isTrialFeasible(initPts,A,b,Aeq,beq,lb,ub,options.TolCon) & ...
         isNonlinearFeasible(C,Ceq,options.TolCon);
...
try
    [x,~,flag] = fmincon(obj,[x0 0],A,b,Aeq,beq,lb,ub,@(y)minFeasConstr(y,nonlcon),options);
