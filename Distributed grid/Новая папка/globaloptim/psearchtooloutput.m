...
%PSEARCHTOOLOUTPUT private to OPTIMTOOL.
...
switch flag
    ...
    case 'iter'
        ...
        if  javaMethodEDT('getChangedState',optimtoolGui) || modelCheck
            ...
            try
                if ~isempty(errOpt)
                    error(message('globaloptim:psearchtooloutput:invalidModel', errOpt));
                end
                
                % Attempt to update changed options
                optnew = optimtoolUpdateChangedOptions(...
                    optnew, optlast, 'patternsearch');
                                
                % Indicate that the options have been updated
                updateoptions = true;
            catch ME
                errordlg(ME.message,'Pattern search run time error');
