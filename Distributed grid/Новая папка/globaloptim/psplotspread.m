function stop = psplotspread(optimvalues,flag)
%PSPLOTSPREAD Plots spread of individuals on Pareto front every iteration.
%   STOP = PSPLOTSPREAD(OPTIMVALUES,FLAG) where OPTIMVALUES is a
...
fileTag = 'psplotspread'; % unique label, usually name of m-file or similar. Allows MATLAB to keep track and update this specific plot.
varTypeX = 'iteration';      % specify that optimvalues.iteration will be plotted on the x-axis
varTypeY = 'spread';  % specify that optimvalues.maxconstraint will be plotted on the y-axis
plotter = @plot; % style of plot we would like to view
markeropts = {'o', 'MarkerSize', 5}; % cell array with name/value pairs for 'plotter' function
xlabelStr = getString(message('globaloptim:psplotcommon:iterationXLabel'));
ylabelStr = getString(message('globaloptim:psplotcommon:spreadYLabel'));
...
if allowRefresh
    ...
    % update title
    titleStr = getString(message('globaloptim:psplotcommon:spreadTitle', num2str(optimvalues.(varTypeY))));
