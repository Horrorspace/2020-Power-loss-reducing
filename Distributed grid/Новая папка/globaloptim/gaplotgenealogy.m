...
%      options = optimoptions('ga','PlotFcn', @gaplotgenealogy );
%    Create an options structure that uses GAPLOTGENEALOGY
%    as the plot function, with 1 as the value of PURGE
%      purge = 1;
%      options = optimoptions('ga','PlotFcn', {@gaplotgenealogy, purge});
%
...
if (isfield(state,'NonlinIneq') || isfield(state,'NonlinEq'))
    msg = getString(message('globaloptim:gaplotcommon:PlotFcnUnavailable','gaplotgenealogy'));
