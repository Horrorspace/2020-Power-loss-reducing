...
if NotVectorized
    for i = 1:maxeval
        [f, count, cache] = funevaluate(FUN, Xin, X(:,i), Cache,cache, cachetol, cachelimit, varargin{:});
...
elseif strcmpi(CompleteSearch,'on')
    [f, count, cache] = funevaluate(FUN, Xin, X(:,1:maxeval), Cache, cache, cachetol, cachelimit, varargin{:});
    FunEval = FunEval+count;
    nextIterate.cache = cache;
    for i =1:maxeval
