function [x,fval,exitFlag,output,population,scores] = gamultiobj(fun,nvars,Aineq,bineq,Aeq,beq,lb,ub,nonlcon,options)
...
%
%
%   See also GA, PATTERNSEARCH, PARETOSEARCH, OPTIMOPTIONS
...
if isfield(options, 'CreationFcn')
    options.CreationFcn = replaceEnumStringWithFcnHdl('GamultiobjOptions', 'CreationFcn', options.CreationFcn);
end
if isfield(options, 'CrossoverFcn')
   options.CrossoverFcn = replaceEnumStringWithFcnHdl('GamultiobjOptions', 'CrossoverFcn', options.CrossoverFcn);
end 
if isfield(options, 'DistanceMeasureFcn')
    options.DistanceMeasureFcn = replaceEnumStringWithFcnHdl('GamultiobjOptions', 'DistanceMeasureFcn', options.DistanceMeasureFcn);
end
if isfield(options, 'HybridFcn')
    options.HybridFcn = replaceEnumStringWithFcnHdl('GamultiobjOptions', 'HybridFcn', options.HybridFcn);
end
if isfield(options, 'MutationFcn')
    options.MutationFcn = replaceEnumStringWithFcnHdl('GamultiobjOptions', 'MutationFcn', options.MutationFcn);
end
if isfield(options, 'PlotFcns')
    options.PlotFcns = replaceEnumStringWithFcnHdl('GamultiobjOptions', 'PlotFcn', options.PlotFcns);
end
if isfield(options, 'SelectionFcn')
    options.SelectionFcn = replaceEnumStringWithFcnHdl('GamultiobjOptions', 'SelectionFcn', options.SelectionFcn);
end
