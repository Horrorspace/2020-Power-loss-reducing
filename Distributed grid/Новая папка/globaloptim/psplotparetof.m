function stop = psplotparetof(optimvalues,flag,valuesToPlot)
%PSPLOTPARETOF Plots the Pareto front up to three objectives.
%   STOP = PSPLOTPARETOF(OPTIMVALUES,FLAG) where OPTIMVALUES is a
...
fileTag = 'psplotparetof'; % unique label, usually name of m-file or similar. Allows MATLAB to keep track and update specific plot.
varType = 'fval'; % specify that optimvalues.(varType) will be plotted
marker = {'pm'}; % Cell array with name/value pairs for 'plot' function
...
elseif size(optimvalues.fval,2) == 1
    ...
    % title string
    titleStr = getString(message('globaloptim:psplotcommon:fvalTitle')); 
end
...
axisLabelStr(1) = getString(message('globaloptim:psplotcommon:objAxisLabelDefault',valuesToPlot(1))); % xlabel
axisLabelStr(2) = getString(message('globaloptim:psplotcommon:objAxisLabelDefault',valuesToPlot(2))); % ylabel

if numel(valuesToPlot) >= 3
    axisLabelStr(3) = getString(message('globaloptim:psplotcommon:objAxisLabelDefault',valuesToPlot(3))); % zlabel (if needed)
end
