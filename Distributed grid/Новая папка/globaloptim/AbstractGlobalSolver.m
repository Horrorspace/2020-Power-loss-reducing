...
%       run             - attempts to search for the best solution of a
...
%PLOTFCNS Alias to PlotFcn.
    ...
    methods
           ...
           % object or parameter of a parameter-value pair            
            if nargin > 0
                ...
                elseif nargin == 1 && ~firstInputObj
                    error(message('globaloptim:AbstractGlobalSolver:invalidInput', ...
                        'MultiStart', 'GlobalSearch'));
                else
                    agsProp = varargin;
                end
                if (nargin > 1) && (mod(length(agsProp),2) ~= 0)
                    error(message('globaloptim:AbstractGlobalSolver:notAPair'));
                end
                while length(agsProp) >= 2
                    prop = agsProp{1};
                    if ~ischar(prop) && ~(isstring(prop) && isscalar(prop))
                        error(message('globaloptim:AbstractGlobalSolver:propertyNameNotString'));
                    end
                    prop = char(prop);
                    val = agsProp{2};
                    ...
                    if strlength(PName) > 0
                        ...
                        % No match at all
                        className = class(obj);
                        error(message('globaloptim:AbstractGlobalSolver:invalidPropertyName', prop, className));
                    end
                end
            end
        end
        function obj = set.Display(obj,value)
            ...
            typeValueChecker('displayType',value);
            obj.Display = char(value);
        end
        function obj = set.FunctionTolerance(obj,value)
            % error check for TolFun property
            typeValueChecker('nonNegReal',value,'TolFun');
        ...
        function obj = set.XTolerance(obj,value)
            ...
            typeValueChecker('nonNegReal',value,'TolX');
        ...
        function obj = set.MaxTime(obj,value)
            ...
            typeValueChecker('nonNegReal',value,'MaxTime');
            obj.MaxTime = value;
        end
        function obj = set.StartPointsToRun(obj,value)
            % error check for StartPointsToRun property
            typeValueChecker('stringsType',value,...
                'StartPointsToRun',{'all','bounds','bounds-ineqs'});
            obj.StartPointsToRun = char(value);
        end
        function obj = set.OutputFcn(obj,value)
            % error check for OutputFcn property
            typeValueChecker('functionOrCellArray',value,'OutputFcns');
            
            % string data type containing multiple strings
            % or just one
            if isstring(value)
                if numel(value) > 1
                    value = cellstr(value);
            ...
            % cell array containing string data types
            elseif iscell(value) 
                convertString = cellfun(@isstring, value);
                value(convertString) = cellfun(@char,  value(convertString), 'UniformOutput', false);
            end
        ...
        function obj = set.PlotFcn(obj,value)
            ...
            typeValueChecker('functionOrCellArray',value,'PlotFcns');
    ...
    methods (Access = protected, Static)
            ...
            if isstruct(problem) && isscalar(problem)
                ...
                if validProblemStruct
                    ...
                    if ~any(strcmpi(problem.solver,validSolvers))
                        ...
                        error(message('globaloptim:AbstractGlobalSolver:run:InvalidSolver', ...
                            validSolversStr));
                    end
                else
                    error(message( ...
                        'globaloptim:AbstractGlobalSolver:run:InvalidStructInput', ...
                          errmsg));
                end
            else
        ...
        function problem = overrideLocalSolverDisplay(problem)
            ...
            % in the createOptimProblem function. As such, if users didn't
            % call createOptimProblem, the local solver display would be
            ...
            %    following:
            %         * An optim.options.SolverOptions object
            %         * A structure
            %         * Empty
            %
            ...
            elseif isempty(problem.options)
                problem.options = optimoptions(problem.solver, 'Display', 'off');
            end
        end
    end    
...
Names = PublicIdents.(Cname).(char(Type));
end
