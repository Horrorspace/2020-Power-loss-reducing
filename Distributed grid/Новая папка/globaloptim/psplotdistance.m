function stop = psplotdistance(optimvalues,flag)
%PSPLOTDISTANCE Plot average distance every Iteration.
%   STOP = PSPLOTDISTANCE(OPTIMVALUES,FLAG) where OPTIMVALUES is a
...
fileTag = 'psplotdistance'; % unique label, usually name of m-file or similar. Allows MATLAB to keep track and update this specific plot.
varTypeX = 'iteration';      % specify that optimvalues.iteration will be plotted on the x-axis
varTypeY = 'averagedistance';  % specify that optimvalues.maxconstraint will be plotted on the y-axis
plotter = @plot; % style of plot we would like to view
markeropts = {'.', 'MarkerSize',5}; % cell array with name/value pairs for 'plotter' function
xlabelStr = getString(message('globaloptim:psplotcommon:iterationXLabel'));
ylabelStr = getString(message('globaloptim:psplotcommon:distanceYLabel'));
...
if allowRefresh
    ...
    % update title
    titleStr = getString(message('globaloptim:psplotcommon:distanceTitle', num2str(optimvalues.(varTypeY))));
