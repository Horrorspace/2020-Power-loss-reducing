...
v(1) = 0.06; v(2) = 0.06; v(3) = 0.09;
 ...
 Cgen = 3890; % стоимость электроэнергии, поступающей от балансирующего узла [руб/МВт*ч]
 Cpot = 1300; % стоимость электроэнергии, отпускаемой на потери электроэнергии в распределительной сети [руб/МВт*ч]
 Tmax = 5240; % Число часов использования максимума нагрузки
 tm = 4500; % Число часов максимума потерь
 Cg = 5.54025; % стоимость газа для ГПУ [руб/куб.м]
 Vg = 250; % расход газа на выработку электроэнергии [куб.м/МВт*ч]
 C_total_gpu = 40*10^6; % суммарная стоимость ГПУ на 1 МВт
 Texp = 30; % срок службы оборудования
 e_disk = 0; % ставка дисконтирования
 Tok = 5; % расчётный срок окупаемости
 Kgpu = C_total_gpu*sum(v); % капитальные вложения в покупку ГПУ
 S_exp = 0.08*Kgpu; % эксплуатационные расходы
 S_amortiz = Kgpu/Texp; % амортизационные отчисления
 Sg = sum(v)*Vg*Cg*Tmax; % расходы на газ [руб]
 Spr = Sg + S_amortiz + S_exp; % Расходы на производство электроэнергии ГПУ
 Kd = 0; % присвоение нулевого значения коэффициенту дисконтирования
 for (t = 1:5)
     Kd = Kd + 1/(1 + e_disk)^t;
 end
 LCOE = 1/Tok*(Kgpu + Spr*Kd); % себестоимость электроэнергии, вырабатываемой ГПУ
 Fitness = 1/Tok*Kd*Cgen*(sum(P))*Tmax + 1/Tok*Kd*Cpot*dP_sum*tm + LCOE; % целевая функция для оптимизации
 LCOE = 1/Tok*(Kgpu + Spr*Kd)/(sum(v)*Tmax); % себестоимость электроэнергии, вырабатываемой ГПУ
 if sum(v)> 2.8
    Fitness = 2*10^16; 
 end
