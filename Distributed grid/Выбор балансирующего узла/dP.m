...
Unom = 10; % номинальное напряжение сети [кВ]
  ...
  S = zeros(1, 33); % создание нулевой матрицы полных мощностей нагрузок узлов
   ...
   Q(32) = 0.1;
   Q(33) = 0.04;
   tg_fi = zeros(1, 33); % создание нулевой матрицы коэффициентов реактивной мощности узлов
   for (i = 2:33) % вычисление коэффициентов реактивной мощности
       tg_fi(i) = Q(i)/P(i); % коэффициент реактивной мощности i-го узла
   end
   for (i = 2:33) % вычисление активных мощностей узлов с учётом распределённой генерации
       P(i) = P(i) - v(i); % активная мощность i-го узла [МВА]
   end
   for (i = 2:33)
      if v(i) == 3.715
          Q(i) = Q(i) - 2.3;
